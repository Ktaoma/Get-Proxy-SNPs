...
SNP_available_in_GSA_array <- fread("input/GSA-24v3-0_A1_b151_rsids.txt") %>% filter(RsID %in% all_rs_id_02$rsid)
...
cdf$cat <- ifelse(cdf$rsid %in% SNP_available_in_GSA_array$RsID,"Available on GSA array",
...
Proxy_SNP_avialable_in_GSA <- cdf %>% filter(cat != "Available on GSA array",type == "Proxy SNPs available") %>%
  dplyr::select(-type)
...
df_rank_final <- df_rank[c(1,4,5,6,7,9,13,14,15,16)]
