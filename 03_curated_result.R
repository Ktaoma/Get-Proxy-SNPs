...
GSA_array <- fread("input/GSA-24v3-0_A1_b151_rsids.txt")[,2] %>%  unlist()

#64 SNPs in summary statistics avaialbe in GSA array
SNP_avialable_in_GSA <- fread("result/final_proxy_EAS_SAS.csv") %>% filter(rsid != ".") %>% filter(rsid %in% GSA_array) %>% select(query) %>% unique() %>% unlist()
...
SNP_not_avialable_in_GSA <- fread("result/final_proxy_EAS_SAS.csv") %>% filter(!query %in% SNP_avialable_in_GSA )
SNP_not_avialable_in_GSA$query %>% unique() %>% length()
...
df_rank <- inner_join(rank_regulome_db,SNP_not_avialable_in_GSA,by="RegulomeDB") %>% filter(RS_Number != ".",Dprime > 0.95)  %>% 
  group_by(query) %>% 
...
SNP_avialable_in_GSA_02 <- fread("result/final_proxy_EAS_SAS.csv") %>% filter(rsid != ".") %>% filter(rsid %in% GSA_array)  %>% filter(Coord %in% SNP_avialable_in_GSA,Dprime == 1)
